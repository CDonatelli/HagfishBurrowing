...
%     
%     answer = questdlg('Do you want to subSample your video', ...
%     'Crop Video', ...
%     'Yes','No','No');
%     switch answer
%         case 'Yes'
%             prompt = {'Enter skip index:'};
%             dlgtitle = 'Input';
%             dims = [1 35];
%             definput = {'2'};
%             answer = inputdlg(prompt,dlgtitle,dims,definput);
%             skipRate = str2num(answer{1});
%         case 'No'
%             skipRate = 1;
%     end
%     dataStruct.skipRate = skipRate;
    
    RawImage = read(fishVideo,1);
    imshow(RawImage)
...
%         end
        ...
        disp(['Processed frame ',num2str(i),'/',num2str(fishVideo.NumberOfFrames  )])
    end
...
%     L = imsegkmeans(single(FrameOut),3); % meant to filter out large
%     objects, takes a super long time (over 2 seconds per frame!)
    [m,n] = size(FrameOut);
...
%Smooth broken bits of fish
    tic
    mask = FrameOut;
    %morphologicalGradient = imsubtract(FrameOut,imerode(FrameOut, strel('Disk',pr5,4)));
    %mask = im2bw(morphologicalGradient,0.03); %This creates outlines
    %mask = imfill(mask, 8, 'holes'); %This fills holes
